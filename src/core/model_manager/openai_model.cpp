...
std::string ModelManager::OpenAI(const std::string &prompt, const std::string &model, const int &max_tokens, const float &temperature)
{
	...
	if (supported_models.find(model) == supported_models.end()) {
		throw std::invalid_argument("Model not supported. Please choose one from the supported list: gpt-4o, gpt-4o-mini, gpt-4-turbo, gpt-4, gpt-3.5-turbo.");
	}
	...
	if (!key) {
		...
		openai::start(); // Assume it uses the environment variable if no API key is provided
	}
