...
inline std::string ConstructPrompt(DataChunk &args,
                                   const nlohmann::json &args_idx) {
  ...
  for (idx_t i = 0; i < args.size(); i++) {
    ...
    filled_template += "Prompt " + std::to_string(i + 1) + ": " +
                       env.render(template_str, (*params)) + "\n\n";
    delete params;
  }

  std::string prompt = env.render_file("src/templates/lf_map_template.txt",
                                       {{"prompts", filled_template}});

  return prompt;
}

static void LfMapScalarFunction(DataChunk &args, ExpressionState &state,
                                Vector &result) {
  ...
  auto max_tokens =
      key_idx_pair["max_tokens"] != -1
          ? args.data[key_idx_pair["max_tokens"]].GetValue(0).GetValue<int>()
          : 100;
  auto temperature =
      key_idx_pair["temperature"] != -1
          ? args.data[key_idx_pair["temperature"]].GetValue(0).GetValue<float>()
          : 1.0;

  std::string llm_response =
      ModelManager::CallComplete(prompt, model_name, max_tokens, temperature);

  // parse the llm response
  std::vector<std::string> response_list =
      CoreLlmResponseParsers::LfMapResponseParser(llm_response, args.size());

  // set the result
  auto index = 0;
  Vector vec(LogicalType::VARCHAR, args.size());
  UnaryExecutor::Execute<string_t, string_t>(
      vec, result, args.size(), [&](string_t _) {
...
void CoreScalarFunctions::RegisterLfMapScalarFunction(DatabaseInstance &db) {
  ExtensionUtil::RegisterFunction(
      db,
      ScalarFunction("lf_map", {}, LogicalType::VARCHAR, LfMapScalarFunction,
